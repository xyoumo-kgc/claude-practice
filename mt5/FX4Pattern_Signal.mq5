...
//+------------------------------------------------------------------+
int ScoreOppForce(int dir, ENUM_TIMEFRAMES bigTF, double atrBig, string &note)
{
...
void CheckPattern(int patternNo, ENUM_TIMEFRAMES bigTF, ENUM_TIMEFRAMES execTF)
{
   ...
   string oppNote;
   int oppScore = ScoreOppForce(dir, bigTF, atrBig, oppNote);
   if(oppScore < 0)
   {
                ...
                string detail, double swingExtreme)
{
   ...
   string msg = StringFormat(
      ...
      _Symbol, grade, score, dirStr, patternName, price, swingExtreme);
